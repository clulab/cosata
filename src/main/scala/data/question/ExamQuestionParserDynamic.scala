...
object ExamQuestionParserDynamic {
  ...
  // Main entry point
  def loadQuestionsFromCSVList(qList:String, fullAnnotation:Boolean = true, noAnnotation:Boolean = false, annotateExplanations:Boolean = true, debugQuestionLimit:Int = 0, tsvMode:Boolean = false):Array[MCQuestion] = {
    toMCQuestion( parse( qList.split(",").toArray, debugQuestionLimit, tsvMode ), fullAnnotation, noAnnotation, annotateExplanations)
  }

  def parse(filenames:Array[String], debugQuestionLimit:Int = 0, tsvMode:Boolean = false):Array[ExamQuestion] = {
    ...
    breakable {
      for (filename <- filenames) {
        val (questionsFile, warningTextFile) = load(filename, debugQuestionLimit, tsvMode)
        questions ++= questionsFile
        warningText.append(warningTextFile)
      }
      // Debug question limit
      if ((debugQuestionLimit > 0) && (questions.size > debugQuestionLimit)) break()
    }
  ...
  def load(filename:String, debugQuestionLimit:Int = 0, tsvMode:Boolean = false):(Array[ExamQuestion], String) = {
    val questions = new ArrayBuffer[ExamQuestion]()
    val warningText = new StringBuilder
    ...
    for (fields <- lines) {
      ...
      breakable {
        ...
        val category                  = safeGetField(LABEL_CATEGORY, headerMap, fields).getOrElse("")
        ...
        if (isMultipleChoice) {
          ...
          val rq = new ExamQuestion(qID, originalID, totalPoints, answerKey, isMultipleChoice,
            hasDiagram, examName, schoolGrade, year, questionType, topics, fold, category, goldExplanation, questionText, mcAnswers,
          ...
          val rq = new ExamQuestion(qID, originalID, totalPoints, answerKey, isMultipleChoice,
            hasDiagram, examName, schoolGrade, year, questionType, topics, fold, category, goldExplanation, question, Array.empty[String],
            flagsCounter, explanationAnnoatators,
            questionSimplified, questionSimplifiedConc, questionFocusPrimary, questionFocusSecondary, answerFocus)
          questions.append(rq)
        }
      ...
      // Debug question limit
      if ((debugQuestionLimit > 0) && (questions.size > debugQuestionLimit)) {
        return (questions.toArray, warningText.toString)
      }
  ...
  def toMCQuestion(in:Array[ExamQuestion], fullAnnotation:Boolean = true, noAnnotation:Boolean = false, annotateExplanations:Boolean = true):Array[MCQuestion] = {
    ...
    for (i <- 0 until in.size) {
      ...
      out(i) = new MCQuestion(question.text, qAnnotation, question.questionType, question.explanation, jAnnotation, question.schoolGrade, topics.toArray, question.fold, question.category, question.qID,
